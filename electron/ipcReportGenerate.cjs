...
module.exports = function () {
  ...
  const getLastReportId = () => {
    try {
      return JSON.parse(fs.readFileSync(lastReportIdPath, "utf-8")).lastId || 0;
    } catch {
      return 0;
    }
  };
  const saveLastReportId = (n) =>
    fs.writeFileSync(lastReportIdPath, JSON.stringify({ lastId: n }), "utf-8");
    ...
    async (
      ...
      return new Promise((resolve) => {
        try {
          ...
          let existingDb = [];
          try {
            existingDb = JSON.parse(
              fs.readFileSync(reportsDbPath, "utf-8") || "[]"
            );
          } catch {}
          ...
          if (wantStart && wantEnd) {
            const isSameRange = (rec) => {
              ...
              // NOTE: If you want different fundingBody to allow duplicates, add it to this comparison.
            ...
            if (existingDb.some(isSameRange)) {
              return resolve({
                success: false,
                error:
                  "You have already generated a report for this spreadsheet with the same date range. Delete the existing report or choose a different date range.",
              });
            }
          }
          ...
          if (typeof fundingBody === "string") {
            positional.push("--fundingBody", fundingBody);
          }
          ...
          function handleClose(code, stdout, stderr) {
            if (code !== 0) {
              return resolve({
                success: false,
                error: (stderr || `Python exited ${code}`).trim(),
              });
            }
            ...
            try {
              ...
              if (parsed && parsed.success === false) {
                return resolve({
                  success: false,
                  error: parsed.error || "Report generation failed.",
                });
              }
              ...
              parsed.spreadsheet_id =
                parsed.spreadsheet_id || spreadsheetId;
              parsed.program_type = parsed.program_type || programType;
              parsed.evaluation_start =
                parsed.evaluation_start ||
                evaluationStartDate ||
                parsed.evaluationStart;
              parsed.evaluation_end =
                parsed.evaluation_end ||
                evaluationEndDate ||
                parsed.evaluationEnd;
          ...
          resolve({
            success: false,
            error: e.message || "Unexpected error in generate-report",
          });
        }
      });
    }
